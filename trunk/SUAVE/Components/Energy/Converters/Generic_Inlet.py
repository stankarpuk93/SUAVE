...
class Pitot_Inlet(Energy_Component):
    ...
    def compute(self, conditions):
        ...
        if compressibility_effects:
            ...
            Mach[i_low] = np.sqrt(
                (((Pt_out[i_low] / Po[i_low]) ** ((gamma[i_low] - 1.) / gamma[i_low])) - 1.) * 2. / (gamma[i_low] - 1.))
            T_out[i_low] = Tt_out[i_low] / (1. + (gamma[i_low] - 1.) / 2. * Mach[i_low] * Mach[i_low])

            # -- Inlet Mach > 1.0, normal shock
            Mach[i_high] = np.sqrt((1. + (gamma[i_high] - 1.) / 2. * Mo[i_high] ** 2.) / (
                        gamma[i_high] * Mo[i_high] ** 2 - (gamma[i_high] - 1.) / 2.))
            T_out[i_high] = Tt_out[i_high] / (1. + (gamma[i_high] - 1.) / 2 * Mach[i_high] * Mach[i_high])
            Pt_out[i_high] = pid * Pt_in[i_high] * ((((gamma[i_high] + 1.) * (Mo[i_high] ** 2.)) / (
                        (gamma[i_high] - 1.) * Mo[i_high] ** 2. + 2.)) ** (gamma[i_high] / (gamma[i_high] - 1.))) * (
                                         (gamma[i_high] + 1.) / (
                                             2. * gamma[i_high] * Mo[i_high] ** 2. - (gamma[i_high] - 1.))) ** (
                                         1. / (gamma[i_high] - 1.))
            P_out[i_high] = Pt_out[i_high] * (1. + (gamma[i_high] - 1.) / 2. * Mach[i_high] ** 2.) ** (
